function p=quantreg_my_self(x,y,tau,order,Nboot)
% Quantile Regression
% 
...
MaxFunEvals = 500;
MaxIter     = 500;
options = optimset('MaxFunEvals', MaxFunEvals,'MaxIter', MaxIter, 'TolFun', 1e-10, 'TolX', 1e-10);


% p = fminsearch(@(p)obj_my_self(p,x,y,tau),pmean);
p = fminsearch(@(p)obj_my_self(p,x,y,tau),pmean, options);
